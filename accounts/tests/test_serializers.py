...
class TestUserSerializer(TestCase):
    ...
    def test_get_datasets_excludes_private_instance_calling_user_is_not_contrib_for(self):
        user = UserFactory()
        ...
        data = serializers.UserSerializer(
            user, context={'user': calling_user}).data
        self.assertNotIn(private.urn, data['experimentsets'])
        self.assertIn(public.urn, data['experimentsets'])
        
    def test_get_datasets_includes_private_instance_calling_user_is_contrib_for(self):
        user = UserFactory()
