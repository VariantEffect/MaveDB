...
from ..views import ManageDatasetUsersView, profile_view, profile_settings
...
class TestProfileSettings(TestCase, TestMessageMixin):
    ...
    def test_cannot_set_invalid_email(self):
        user = UserFactory()
        request = self.create_request(
            method="post", path="/profile/", data={"email": "not an email.com"}
        )
        request.user = user
        response = profile_settings(request)
    ...
    def test_users_email_appears_in_profile(self):
        user = UserFactory()
        request = self.create_request(method="get", path="/profile/")
        request.user = user
        response = profile_settings(request)
...
class TestProfileManageInstanceView(TestCase, TestMessageMixin):
    ...
    def setUp(self):
        ...
        self.path = reverse_lazy(
            "accounts:manage_instance", args=(self.instance.urn,)
        )
        self.client.logout()
