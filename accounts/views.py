...
def delete(urn, request):
    ...
    if isinstance(instance, (Experiment, ExperimentSet)):
        message = "Child {} must be deleted prior to deleting this {}"
        if instance.children.count():
            if isinstance(instance, Experiment):
                message.format("Score Sets", 'Experiment')
            if isinstance(instance, ExperimentSet):
                message.format("Experiments", 'Experiment Set')
...
def publish(urn, request):
    ...
    failed = instance.processing_state == constants.processing
    if failed:
        messages.error(
            request,
