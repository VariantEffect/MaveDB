...
@celery_app.task(bind=True, ignore_result=True, base=BasePublishTask)
...
@celery_app.task(bind=True, ignore_result=True, base=BaseDeleteTask)
...
def create_variants(
    ...
    logger.info(
        "Sending scores dataframe with {} rows.".format(len(scores_records))
    )
    logger.info(
        "Sending counts dataframe with {} rows.".format(len(counts_records))
    )
