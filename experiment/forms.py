...
from .models import Keyword, TargetOrganism, SraIdentifier
from .models import PubmedIdentifier, DoiIdentifier
...
from .validators import (
    ...
    validate_scoreset_json, validate_variant_json, validate_hgvs_string,
    validate_csv_extension, validate_sra_list, validate_doi_list,
    validate_pubmed_list, validate_wildtype_sequence, validate_target,
    validate_target_organism, validate_variant_rows, validate_keyword_list
)
...
class ScoreSetForm(DatasetModelForm):
    ...
    def set_replaces_options(self):
        pks = [
            i.pk for i in self.user.profile.administrator_scoresets()
            if i in self.experiment.scoreset_set.all() and i != self.instance
        ]
        scoresets = ScoreSet.objects.filter(pk__in=set(pks)).order_by("urn")
        self.fields["replaces"].queryset = scoresets
    ...
    def set_experiment_options(self):
        ...
        pks = [i.pk for i in choices]
        experiments = Experiment.objects.filter(pk__in=set(pks)).order_by("urn")
        self.fields["experiment"].queryset = experiments
